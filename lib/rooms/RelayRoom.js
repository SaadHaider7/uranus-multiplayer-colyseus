    ...
    onCreate(options) {
        ...
        this.onMessage('*', (client, type, message) => {
            ...
            if (type === 'requestAvatarUrl' && message.sessionId) {
                ...
                if (player)
                    client.send(`receiveAvatarUrl:${message.sessionId}`, [client.sessionId, { avatarUrl: player.avatarUrl }]);
                return;
            }
            // --- filter messages if required
            if (type === 'chat:message' && message.group) {
                // --- if this is a group chat, broadcast only to group users
                message.group.forEach((clientId) => {
                    const receiverClient = this.clients.find(o => o.id === clientId);
                    if (receiverClient)
                        receiverClient.send(type, [client.sessionId, message]);
                });
                return;
            }
